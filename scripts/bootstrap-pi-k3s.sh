#!/usr/bin/env bash
# bootstrap-pi-k3s.sh – Chrony + k3s auf Raspberry Pi Nodes installieren
# Voraussetzungen: nodes.env ausgefüllt, SSH-Pubkey auf allen Nodes vorhanden
# Ausführen via: mise run pi-up
set -euo pipefail
...
[[ -f "${NODES_ENV}" ]] || { echo "Fehler: ${NODES_ENV} nicht gefunden"; exit 1; }
...
# Chrony ist für k3s-Cluster essenziell: stark abweichende Uhren führen zu
# TLS-Fehlern und fehlgeschlagenen Certificate-Checks zwischen den Nodes.
...
if grep -q 'REPLACE_WITH_PI_SERVER_IP' "${CILIUM_MANIFEST}"; then
  ...
  echo
  echo "  Cilium HelmRelease aktualisiert (k8sServiceHost: ${PI_SERVER})"
  echo "  Bitte committen bevor pi-flux-bootstrap:"
...
echo
echo "Weiter mit:"
echo "  mise run pi-cilium-up"
echo "  # dann committen falls cilium.yaml geändert"
echo "  mise run pi-flux-bootstrap"
